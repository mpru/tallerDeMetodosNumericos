---
title: "Algoritmos"
subtitle: "Unidad 3: Solución de Sistemas de Ecuacioens Lineales"
...
\begin{algorithm}[H]
	...
	\begin{algorithmic}
		...
		\STATE x $\leftarrow$ vector numérico de largo n
		\STATE x[n] $\leftarrow$ b[n] / A[n, n]
		\FOR {k desde n-1 hasta 1 cada -1}
			\STATE x[k] $\leftarrow$ (b[k] - A[k, k+1 : n] * x[k+1 : n]) / A[k, k]
		\ENDFOR
		\RETURN x
	\end{algorithmic}
\end{algorithm}
	...
	\begin{algorithmic}
		...
		\FOR {q desde 1 hasta n-1}
			\FOR {r desde q+1 hasta n}
				...
				\STATE Aum[r, q:(n+1)] $\leftarrow$ Aum[r, q:(n+1)] - mrq * Aum[q, q:(n+1)]
			\ENDFOR
		\ENDFOR
		\STATE 
		...
		\STATE A $\leftarrow$ Aum[, 1:n]
		\STATE b $\leftarrow$ Aum[, n+1]
		\STATE x $\leftarrow$ susreg(A, b)
		\RETURN x
	\end{algorithmic}
\end{algorithm}
...
\begin{algorithm}[H]
	...
	\begin{algorithmic}
		...
		\STATE r $\leftarrow$ loc(sr2 = max(sr2))[1] + q - 1
		\STATE \textit{Intercambiar filas:}
		\STATE temp $\leftarrow$ Aum[q, ]
		\STATE Aum[q, ] $\leftarrow$ Aum[r, ]
		\STATE Aum[r, ] $\leftarrow$ temp;
...
\begin{algorithm}[H]
	...
	\begin{algorithmic}
		...
		\FOR {r desde 1 hasta n}
		    \IF {r = q}
		        \STATE Aum[q, q:(n+1)] $\leftarrow$ Aum[q, q:(n+1)] / Aum[q, q]
		    \ELSE
    	    	\STATE mrq $\leftarrow$ Aum[r, q] / Aum[q, q]
                \STATE Aum[r, q:(n+1)] $\leftarrow$ Aum[r, q:(n+1)] - mrq * Aum[q, q:(n+1)]
            \ENDIF
